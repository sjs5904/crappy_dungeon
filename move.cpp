...
void do_combat(dungeon *d, character *atk, character *def)
{
  ...
  if (def->alive) {
    ...
    if (atk == d->PC) {
      // d->num_monsters--;
      for(i=0; i < d->PC->equipSlots.size()-1; i++){
        if(d->PC->equipSlots[i] != NULL)
          damageDone += d->PC->equipSlots[i]->roll_dice();
      }
      damageDone += atk->damage->roll();

      if(damageDone > def->hp){
        //pc killed monster
        def->alive = 0;
        charpair(def->position) = NULL;
        d->num_monsters--;
        io_queue_message("%s smites the %s", atk->name, character_get_name(def));
        atk->kills[kill_direct]++;
        atk->kills[kill_avenged] += (def->kills[kill_direct] +
                                     def->kills[kill_avenged]);
      }else{
        def->hp -= damageDone;
        io_queue_message("Hit %s for %u damage", character_get_name(def), damageDone);
      }
    } else {
      damageDone += atk->damage->roll();
      ...
      if(damageDone > def->hp){
        ...
        def->hp -= damageDone;
        io_queue_message("%s took %u damage", def->name, damageDone);
      }
      /* Queue an empty message, otherwise the game will not pause for *
       * player to see above.                                          */
      io_queue_message("");
...
void move_character(dungeon *d, character *c, pair_t next)
{
  if (charpair(next) &&
    ...
    if(d->map[character_get_y(c)][character_get_x(c)] != ter_water && c->wet){
      c->speed += 2;
      c->wet = false;
    }else if(d->map[character_get_y(c)][character_get_x(c)] == ter_water && !c->wet){
      c->speed -= 2;
      if(c->speed < 1){
        c->speed = 1;
      }
      c->wet = true;
    }else if(d->map[character_get_y(c)][character_get_x(c)] == ter_lava){
      if(c == d->PC) {
        io_queue_message("it burns!");
      }
      if(2 > c->hp){
        if(c == d->PC){
          c->alive = 0;
    ...
    if(c == d->PC){
      if(d->PC->regen == 5 && d->PC->hp != d->PC->maxHP){
        d->PC->hp += 2;
        d->PC->regen = 0;
        if(d->PC->hp > d->PC->maxHP){
          d->PC->hp = d->PC->maxHP;
        }
        io_queue_message("regenerated %s hp: %d", c->name, c->hp);
      }
      d->PC->regen ++;
    }
  }
